...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes = ["/", "/explore", "/shows", "/posters", "/artists", "/venues", "/about"].map(
    (path) => url(path),
  );
  ...
  try {
    const [posters, artists, venues, shows] = await Promise.all([
    ...
    const showIds = new Set(
      shows.map((s) => s.id).filter((id): id is string => Boolean(id)),
    );
